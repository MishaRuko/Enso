...
from .models import FurnitureItem, ModelFile, ProductImage, StockInfo, Variant
...
def _parse_product(product: dict[str, Any]) -> FurnitureItem:
    ...
    return FurnitureItem(
        ...
        description=(
            f"{product.get('name', '')} {product.get('typeName', '')},"
            f" {product.get('itemMeasureReferenceText', '')}"
        ).strip(", "),
...
async def get_3d_models(item_code: str) -> list[ModelFile]:
    """Fetch 3D model URLs for an item via the IKEA Rotera endpoint."""
